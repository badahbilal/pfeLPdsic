...
@section('content')
    <div class="box box-primary" style="    padding-bottom: 3px"><h3 class="text-center">liste des produits</h3></div>
    <div style="margin-bottom: 10px;">
        <a style="padding: 13px;font-size: 17px;font-weight: bold;" href="produits/create" class="btn btn-primary">
    ...
    <div class="box box-primary">
        ...
        <div class="box-body">
            <table id="dataTableProduis" style="width:100%" class="order-column mdl-data-table table table-bordered table-striped">
                ...
                <tr>
                    <td><img width="60" height="60" src="{{'http://127.0.0.1:8000/storage/produitsPhotos/'.$produit->photoProd}}"></td>
                    <td data-search="Donna Snider">{{$produit->idProd}}</td>
                    <td>{{$produit->nomProd}}</td>
                    <td>
                        @foreach($categories as $categorie)
                            @if($produit->idCat === $categorie->idCat)
                                {{$categorie->nomCat}}
                            @endif
                        @endforeach
                    </td>
                    <td data-order="{{$produit->prixAchatProd}}">{{$produit->prixAchatProd}}</td>
                    <td data-order="{{$produit->prixVenteProd}}">{{$produit->prixVenteProd}}</td>
                    <td data-order="{{$produit->qteStockProd}}">{{$produit->qteStockProd}}</td>
